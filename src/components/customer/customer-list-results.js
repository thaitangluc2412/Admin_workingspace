...
export const CustomerListResults = (props) => {
  ...
  let result = [];
  useEffect(()=>{
    axios.get(`128.199.166.110:8080/api/customer/page?page=${page-1}&size=${SIZE}`).then(res => {
    setCount(Math.ceil(parseInt(res.data.data.count)/SIZE));
    setCustomers(res.data.data.objectList);
  })
  },[])
  useEffect(()=>{
    axios.get(`128.199.166.110:8080/api/customer/search?value=${a}&page=${page-1}&size=${SIZE}`).then(res => {
      setCustomers(res.data.data.objectList);
  ...
  useEffect(()=>{
    axios.get(`128.199.166.110:8080/api/customer/searchAll?value=${a}`).then(res => {
      setCount(Math.ceil(parseInt(res.data.data.count)/SIZE));
    })
  },[a])
